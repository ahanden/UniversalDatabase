...
sub parse {
    ...
    my $pathway_query     = $dbh->prepare("INSERT INTO pathways (`name`, `database`, `external_id`) VALUES (?, 'REACTOME', ?)");
    my $uniprot_query     = $dbh->prepare("SELECT entrez_id FROM gene_xrefs WHERE Xref_db = 'UniProt' AND Xref_id = ?");
    my $symbol_query      = $dbh->prepare("SELECT entrez_id FROM genes WHERE symbol = ? AND tax_id = (SELECT tax_id FROM taxonomies WHERE name = ?)");
    my $participant_query = $dbh->prepare("INSERT IGNORE INTO pathway_participants (pathway_id, entrez_id) VALUES (?, ?)");
    my $progress = 0;
    foreach my $pathway(values(%pathways)) {
        print STDERR "$progress/".scalar(keys(%pathways))."\r";
        $progress++;
